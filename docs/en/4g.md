...
> **Info** A Raspberry Pi camera device `/dev/video0` could be used by `droid` systemd service. In this case `gst-rtsp-launch` will not get an access to the device. For stop the `droid` run command `sudo systemctl stop droid`.
