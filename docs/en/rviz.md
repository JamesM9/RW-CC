...
> **Hint** You can use the [preconfigured virtual machine image](simulation_vm.md) with ROS and droid toolkit.
...
To start the droid state visualization in real time, connect to it [via Wi-Fi](wifi.md) (`droid-xxxx`) and run rviz, specifying an appropriate ROS_MASTER_URI:
...
To start rqt for monitoring droid status, use command:
