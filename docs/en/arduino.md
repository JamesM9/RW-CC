...
Arduino is to be installed on droid and connected via a USB port.
...
To start the link with Arduino at the startup automatically, set argument `arduino` in the droid launch file (`~/catkin_ws/src/droid/droid/launch/droid.launch`):

```xml
<arg name="arduino" default="true"/>
```

After the launch file is edited, restart the `droid` service:
...
// Connecting droid and MAVROS package message header files
