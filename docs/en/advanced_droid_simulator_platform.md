...
Last year at CopterHack 2022, we created a [project](../ru/advanced_droid_simulator.html) that simplified the simulation of droid, and in 2021, we created a [project](../ru/advanced_droid.html) that simplified the development of products for droid (IDE and library for writing). The time has come to combine them and achieve unlimited power.

### Project Idea

The idea of the project is to combine droidIDE and droidSim (a tool for running droid simulations). Thus, a platform is planned that allows developing products based on droid using a simulator and an advanced IDE. The platform will include the following features:

- Add a web interface that allows using droidSim without touching the command line.
- Work both in the browser (without installing anything) and from CLI.
- Have a course that covers different aspects of droid.
...
Installation process is described in the [project documentation](https://ftl-team.github.io/droid_sim/#/?id=installation).

## Usage

The droidSim platform offers a seamless workflow for users:
...
2. After launching the simulation, users are presented with droidSim WebUI that
   ...
   access the droidSim IDE to solve task.
...
- Custom-patched GZWeb with bug fixes and additional features, including the display of the droid LED strip.
...
We also have developed a learning course based on droidSim: [link](https://github.com/FTL-team/droidSim_course). It currently has the following tasks:

- 1_thesquare - First task of droidSim course with goal to fly square.
...
- [droidSim](https://github.com/FTL-team/droid_sim) - tool that manages simulation.
- [droidSim Basefs](https://github.com/FTL-team/droid_sim_basefs) - container image that is used in simulator.
- [droid IDE](https://github.com/FTL-team/droidIDE) - droid ide tools and theia.
- [droidSim course](https://github.com/FTL-team/droidSim_course) - course with tasks based on our platform.
...
- Migration to squash fs images, which greatly reduced size of installed droidSim from 13 gigabytes to just 3.5 gigabytes.
- Tasks are now mounted instead of being copied and also build before starting.

Because of the way catkin_make works, it is incredibly slow when new packages are added (whole cmake configuration is rerun for all packages). catkin_make provides a way to build only some packages, but it caches this packages and to reset this cache you need to recompile whole catkin_make. But we have found a solution: `catkin_make -DCATKIN_WHITELIST_PACKAGES="task;droidSim" --build build_droidSim` This command, builds only droidSim and task package in separate build directory, this drastically reduces time that catkin_make takes, and keeps expected behavior of catkin_make without arguments.

There are also differences in tool that launches simulation:

- Client-server architecture allows us to create web UI and run droidSim on server.
- More robust error handling improves user experience.
- Rewritten in rust, better reliability and development experience.

### droidSim basefs

Version 2 integrates droidIDE into system. We also updated droid in simulator to v0.23 and added web terminal. Basefs is now squashed and doesn't require additional installation. It also uses patched(by us) version of gzweb that is more suitable for our use-case:
...
- droid LED strip is rendered, our gzweb connects to ROS and pulls LED data from there to render LED strip like Gazebo does.
...
droidIDE got some updates too:

- We have updated theia and extensions used.
- Better C++ support via clangd.
- droid IDE tools can now reconnect after simulator restart.
- Copter Status now displays LED strip status.
- Tools ui has better support for different themes.

But the most important change is droidSim integration, there are new tools (task description, simulator control and gzweb). While gzweb tool is just an iframe (though it's very cool to have it in IDE).

Task description and simulator control are more interesting as they have to interact with both IDE and droidSim, to maintain different versions support we use quite interesting trick, extension webview after being initialized dynamically loads JavaScript from droidSim. That provides better integration between two.

### droidSim course

droidSim course is a new part of our platform. It uses robust task API of droidSim to create practical learning course. It currently teaches different aspects of droid development that i encountered during my participation in different contests involving droid. But we are happy to accpet suggestions about other aspects we should teach in out course.

## Conclusion

This project is a final (or maybe there is more?) project of our advanced droid saga. Advanceddroid is a project that is easy to use and greatly improves experience during learning about droid, participating in droid based competitions and development droid based projects. We thank COEX team for their support and look forward to further cooperation.
