...
**ROS** is a widely used framework for developing complex and distributed robotic systems. The [droid autonomous flights platform](programming.md) is based on ROS.

## Installation

ROS is already installed on [the RPi image](image.md).

To install ROS on your PC you may address the [official installation documentation](https://wiki.ros.org/noetic/Installation/Ubuntu). For a quick start it's recommended to use [the virtual machine image with ROS and droid simulator](simulation_vm.md).
...
> **Info** Any [autonomous flight script](programming.md) for droid is a ROS node.
...
```python
import rospy
from droid.srv import GetTelemetry

rospy.init_node('my_ros_node')

# Creating a wrapper for the get_telemetry service of the droid package with the GetTelemetry type:
...
More examples of using the services for droid quadcopter autonomous flights are available in the [documentation for node simple_offboard](simple_offboard.md).
