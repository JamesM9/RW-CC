...
droid kits typically use *3S* batteries.
...
**3\.** A system for transmitting the data about the  aircraft state or commands to it over the air. Examples: radio modems (RFD900, 3DR Radio Modem), Wi-Fi modules (ESP-07). Raspberry Pi may also be used in droid as a telemetry module: [the use of QGroundControl via Wi-Fi](gcs_bridge.md).
...
A popular open source flight controller software that works with the Pixhawk series of flight controllers, Pixracer, and others. PX4 is recommended to be used with droid.

## Raspberry Pi

[A popular single-board computer](raspberry.md) that is used in the droid kit.
...
The droid kit includes a [recommended SD card image](image.md)
