...
![TF2 droid frames](../assets/frames.png)

Main frames in the `droid` package:
...
tf2 ROS package is used extensively in the droid platform. tf2 is a set of libraries for C++, Python and other programming languages that are used to work with the frames. Internally, ROS nodes publish `TransformStamped` messages to `/tf` topic with transforms between frames at certain points in time.
