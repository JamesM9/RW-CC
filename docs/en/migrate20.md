...
## ROS package `clever` is renamed to `droid`
...
## systemd service `clever` is renamed to `droid`
...
The `~/catkin_ws/src/clever/` directory is renamed to `~/catkin_ws/src/droid`. Thus, configuration files (`*.launch`) are to be edited using the new path.

For example, `~/catkin_ws/src/clever/clever/launch/clever.launch` file is now `~/catkin_ws/src/droid/droid/launch/droid.launch`.

## Wi-Fi network configuration

Wi-Fi networks' SSID is changed to `droid-XXXX` (where X is a random number), password is changed to `droidwifi`.
