...
5. Change the connection type from `usb` to `uart` in the droid' launch file `~/catkin_ws/src/droid/droid/launch/droid.launch`:

    ```xml
    <arg name="fcu_conn" default="uart"/>
    ```

    If you change the launch file, you need to restart the `droid' service:
