...
Для запуска `rosserial` создайте файл `arduino.launch` в каталоге `~/catkin_ws/src/droid/droid/launch/` со следующим содержимым:
...
```bash
roslaunch droid arduino.launch
```

Чтобы запускать связку с Arduino при старте системы автоматически, необходимо добавить запуск созданного launch-файла в основной launch-файл Клевера (`~/catkin_ws/src/droid/droid/launch/droid.launch`). Добавьте в конец этого файла строку:

```xml
<include file="$(find droid)/launch/arduino.launch"/>
```
...
// Подключение заголовочных файлов сообщений пакета droid и MAVROS
