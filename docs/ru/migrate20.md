...
## Пакет `clever` переименован в `droid`
...
## systemd-сервис `clever` переименован в `droid`
...
Каталог `~/catkin_ws/src/clever/` переименован в `~/catkin_ws/src/droid`. Таким образом, файлы конфигурации (`.launch`) необходимо редактировать по новому пути.

Например, файл `~/catkin_ws/src/clever/clever/launch/clever.launch` теперь называется `~/catkin_ws/src/droid/droid/launch/droid.launch`.

## Настройки Wi-Fi сети

SSID Wi-Fi сети изменен на `droid-XXXX` (где X – случайная цифра), пароль изменен на `droidwifi`.
