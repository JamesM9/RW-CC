...
Страница должна быть помещена в каталог `/home/pi/catkin_ws/src/droid/droid/www/`. После этого она станет доступна по адресу `http://192.168.11.1/droid/<имя_страницы>.html`. При открытии страницы браузер должен показать окно с телеметрией дрона, а также постоянно выводить состояние полетного контроллера в консоль.
...
Смотрите также пример реализации ([`gcs.html`](https://github.com/CopterExpress/droid/blob/master/droid/www/gcs.html), [`gcs.js`](https://github.com/CopterExpress/droid/blob/master/droid/www/js/gcs.js)) упрощенной браузерной наземной станции (GCS) на Клевере по адресу http://192.168.11.1/droid/gcs.html.
