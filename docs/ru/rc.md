...
Установите [образ droid на RPi](image.md). Для работы приложения параметры `rosbridge` и `rc` в launch-файле (`~/catkin_ws/src/droid/droid/launch/droid.launch`) должны быть включены:
...
Подключите смартфон к [Wi-Fi](wifi.md) сети Клевера (`droid-xxxx`). Приложение должно подключиться с коптеру автоматически. При успешном подключении должны отобразиться текущий [режим](modes.md) и заряд батареи.
