...
5. Поменяйте в launch-файле Клевера (`~/catkin_ws/src/droid/droid/launch/droid.launch`) тип подключения с `usb` на `uart`:

    ```xml
    <arg name="fcu_conn" default="uart"/>
    ```

    При изменении launch-файла необходимо перезапустить сервис `droid`:
