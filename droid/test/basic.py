...
@pytest.fixture()
def node():
    return rospy.init_node('droid_test', anonymous=True)
...
def test_blocks(node):
    rospy.wait_for_service('droid_blocks/run', timeout=5)
    rospy.wait_for_service('droid_blocks/stop', timeout=5)
    rospy.wait_for_service('droid_blocks/load', timeout=5)
    rospy.wait_for_service('droid_blocks/store', timeout=5)

    from std_msgs.msg import String
    from droid_blocks.srv import Run

    def wait_print():
        try:
            wait_print.result = rospy.wait_for_message('droid_blocks/print', String, timeout=5).data
    ...
    run = rospy.ServiceProxy('droid_blocks/run', Run)
